...
When `CGMINER_MONITOR_RESTART_SCHEDULE_URL` is set, `RestartScheduleClient` polls `cgminer_manager`'s `GET /api/v1/restart_schedules.json` (one HTTP call cached for 30 s) and AlertEvaluator skips the `offline` rule for any miner currently inside `[scheduled_minute, scheduled_minute + RESTART_WINDOW_GRACE_SECONDS)` UTC. Emits `alert.suppressed_during_restart_window` once per miner per tick when it fires. The fetch is fail-open: a manager outage yields an empty schedule map and monitor falls back to alerting normally — operators get paged on a real outage even when the manager is itself down. Keeps monitor's pure-read posture intact; the only writes happen in the manager.
